...
int main()
{
  ...
  // Statement Data:
    int deposits, clients; scanf("%d %d", &deposits, &clients);
    double installationCosts[deposits]; for (int i = 0; i < deposits; i ++) scanf("%lf", &installationCosts[i]);
  ...
  // Decision Variables:
    ...
    for (int i = 0; i < deposits; i ++) serveClient[i] = IloBoolVarArray(env, clients);
  ...
  // Restrictions:
    // Service:
      ...
      {
        IloExpr serve(env);
        for (int i = 0; i < deposits; i ++) serve += serveClient[i][j];
    ...
    // Installation:
      ...
      {
        IloExpr install(env);
        for (int j = 0; j < clients; j ++) install += serveClient[i][j];
